<template>
    <div>
        <!--添加车辆弹窗-->
        <el-dialog class="capacity-pop capacity-line capacity-pop-big" title="添加车辆"  :visible.sync="visible" :carRescuePkId="carRescuePkId"
        ...
        methods: {
            ...
            updateSuccess(response, file) {
                ...
                if (attrs) {
                    if ("driving_license_front" === imageType) { //行驶证正面
                        this.addCarRescueForm.dlPlateNo = attrs['vehiclePlateNumber'];          // 车牌号码
                        this.addCarRescueForm.dlCarType = attrs['vehicleType'];                 // 车辆类型
                        this.addCarRescueForm.dlOwner = attrs['vehicleOwner'];                  // 所有人
                        ...
                        this.addCarRescueForm.dlBrand = attrs['vehicleModel'];                  // 品牌型号
                        this.addCarRescueForm.dlEngineNo = attrs['vehicleEngineNumber'];        // 发动机号码
                        this.addCarRescueForm.dlCarVin = attrs['vehicleVinNumber'];             // 车架号
                        this.addCarRescueForm.dlRegisterTime = attrs['vehicleRegisterDate'];    // 注册日期
                        this.addCarRescueForm.dlIssueTime = attrs['vehicleIssueDate'];          // 发证日期
                    } else if ("driving_license_second" === imageType) {//行驶证背面
                        this.addCarRescueForm.dlHostlingWeight = attrs['vehicleCurbWeight'];                // 整备质量
                        this.addCarRescueForm.dlCheckEndTime = attrs['vehicleEffectiveUntil'];              // 检验有效期止
                        this.addCarRescueForm.dlLoadWeight = attrs['vehicleLoadrWeight'];                   // 核定载质量
                        this.addCarRescueForm.dlScrapEndTime = attrs['vehicleNote'];                        // 强制报废日期止
                        this.addCarRescueForm.dlGabarite = attrs['vehicleOutSize'];                         // 外廓尺寸
                        this.addCarRescueForm.dlTractionWeight = attrs['vehicleTowWeight'];                 // 准牵引质量
                    }
                }
            ...
            submitAddCarRescueForm() {
                ...
                this.$refs.addCarRescueForm.validate((valid) => {
                    if (valid) {
                        if(!that.addCarRescueForm.driving_license_second_imageUrl){
