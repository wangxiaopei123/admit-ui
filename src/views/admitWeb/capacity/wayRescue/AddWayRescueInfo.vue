							...
							<div class="car-way-table" v-if="showWayRescueTable2===1">
								<el-table
									...
									<el-table-column fixed="right" label="操作" width="180">
										<template slot-scope="scope">
											<el-button type="text" size="small" @click="uploadPapers=true,uploadCarRescuePkId=scope.row.pkId" v-if="scope.row.isUpPic!=1">上传证件{{scope.row.isUpPic}}</el-button>
                ...
                this.$get("ltdCarRescue/queryCarRescueImg",data).then((response) => {
                    ...
                    if (response.code != 1 ) {
                        ...
                        if(result!=null && result.companyImgList!=null && result.companyImgList.length>0){
							...
							console.log("result.companyImgList",result.companyImgList);
