<template>
	<div class="case">
								...
								<div class="search-manage small-search small-car-search clear">
									<div class="search-l clear">
										...
										<el-form-item class="search-date" label="更新时间" prop="">
											<el-date-picker type="date" placeholder="开始日期" v-model="selectCarList.modifyTimeStart">
											</el-date-picker>
											<span>至</span>
											<el-date-picker type="date" placeholder="结束日期" v-model="selectCarList.modifyTimeEnd">
											</el-date-picker>
										</el-form-item>
								...
								<el-table
									...
									<el-table-column fixed="right" label="操作" width="220px">
										<template slot-scope="scope">
											...
											<el-button  type="text" @click="removeTrailer(scope.row.dlPlateNo)">解绑挂车</el-button>
						...
						<el-tab-pane name="second" label="车头列表">
							...
							<el-form label-width="80px" class="demo-ruleForm" :model="selectCarHeadsList">
								<div class="search-manage small-search small-car-search clear">
									<div class="search-l clear">
										...
										<el-form-item class="search-date" label="更新时间" prop="">
											<el-date-picker type="date" placeholder="开始日期" v-model="selectCarHeadsList.modifyTimeStart">
											</el-date-picker>
											<span>至</span>
											<el-date-picker type="date" placeholder="结束日期" v-model="selectCarHeadsList.modifyTimeEnd">
											</el-date-picker>
										</el-form-item>
										<el-form-item label="板位数">
							...
							<div class="car-way-table" v-if="headStocktableData !== null">
								<el-table
									...
									<el-table-column label="审核状态" width="120px" prop="">
										审核中
									</el-table-column>
									<el-table-column label="审核时间" width="160px"  prop="">
									</el-table-column>
									<el-table-column
											label="更新时间"
											width="160px"
									...
									<el-table-column fixed="right" label="操作" width="220px">
										<template slot-scope="scope">
											...
											<!--<el-button type="text" @click="findCarHeadInfo(scope.row.headStockPkId,'2','更新保险信息')">更新保险</el-button>
											<el-button type="text" @click="findCarHeadInfo(scope.row.headStockPkId,'1','编辑车辆信息')">编辑</el-button>
						...
						<el-tab-pane name="third" label="挂车列表">
							...
							<div class="car-way-table" v-if="trailertableData  !==null">
								<el-table
									...
									<el-table-column label="审核状态" width="120px">审核中
									</el-table-column>
									...
									<el-table-column fixed="right" label="操作" width="220px">
										<template slot-scope="scope">
											...
											<!--<el-button type="text" @click="findCarTrailerInfo(scope.row.pkId,'2','更新保险信息')">更新保险</el-button>
											<el-button type="text" @click="findCarTrailerInfo(scope.row.pkId,'1','编辑车辆信息')">编辑</el-button>
											<el-button type="text" size="small">删除</el-button>-->
										</template>
									</el-table-column>
								</el-table>
		...
		<el-dialog class="capacity-pop capacity-line capacity-pop-big" title="添加/更换司机" :visible.sync="changeDriver" width="34%" center>
			<div class="capacity-pop-in">
				<el-form :model="changeDriverForm" :rules="changeDriverRules" ref="changeDriverForm" label-width="110px" class="demo-ruleForm">
					...
					<el-form-item label="副司机" prop="driver2">
		...
		<el-dialog :title="CarChengeTitle" :visible.sync="carHeadBindingVisible" width="34%" center>
			<el-form :model="carHeadBindingModel" :rules="carHeadBindingRules"
				...
				<el-form-item label="挂车牌号" prop="dlPlateNo">
					<el-select  v-model="carHeadBindingModel.dlPlateNo" placeholder="请选择挂车" width="80%">
						<el-option
					...
					<div v-if="carDialogType === '1'">
						...
						<div class="info-item clear">
							...
							<el-form-item label="车头车牌号码：">
								{{carTrailerInfoModel.headstockId}}
							</el-form-item>
							<el-form-item label="所有人：">
		...
		<AddBigCarHead :visible.sync="addHeadstock" pkId=""  title="牵引车 中置轴前车——编辑"></AddBigCarHead>
		<AddBigCarTrailer :visible.sync="addTrailer" pkId="" title="半挂车 中置轴后挂——编辑"></AddBigCarTrailer>
	...
	export default {
		...
		data() {
			...
			return {
				...
				changeDriverForm:{headstockId:"",driver1:"",driver2:""},
				changeDriverRules:{
		...
		methods: {
			...
			findCarTrailerInfo(pkId,type,title){
				...
				that.$get("ltdCarTrailer/selectOne", {"pkId": pkId}).then(function (response) {
				    console.log(response.response.ltdCompanyImgs+"----")
					if (response.response != null) {
						that.updateCarTrailerInfoForm = response.response.ltdCarTrailer;
						console.error(response.response.ltdCarTrailer)
			...
			openCarDetailsInfoMobel(pkId, type,title) { // 车头——车辆详情信息弹窗---数据绑定
				...
				that.$get("ltdCarHeadstock/selectOne", {"pkId": pkId}).then(function (response) {
					...
					if (response.response.ltdCarHeadstock != null) {
						...
						console.error(response.response.ltdCompanyImgs);
						if (response.response.ltdCompanyImgs.length > 0) {
							response.response.ltdCompanyImgs.forEach(function (image) {
			...
			findCarList(){
				...
				console.error(this.selectCarList)
				// this.reload();
			},
			findCarHeadList(){
				let that=this;
