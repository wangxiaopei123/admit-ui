	...
	<div class="case">
								...
								<div class="search-manage small-search small-car-search clear">
									<div class="search-l clear">
										...
										<el-form-item label="审核状态">
											<el-select v-model="searchAuthState"></el-select>
										</el-form-item>
									...
									<el-table-column fixed="right" label="操作" width="220">
										<!--<template slot-scope="scope">
											...
											<el-button type="text" size="small" v-if="scope.row.isBindCar==0"
													   @click="changeCarFun(scope.row.pkId, scope.row.driverType,0)">
												关联车辆
											</el-button>
											<el-button type="text" size="small" v-if="scope.row.isBindCar==1"
													   @click="changeCarFun(scope.row.pkId, scope.row.driverType,1)">
												更换车辆
											</el-button>
										</template>-->
									</el-table-column>
		...
		<el-dialog class="capacity-pop capacity-line" title="司机信息批量导入" :visible.sync="bulkImport" width="40%" center>
			<div class="capacity-pop-in">
				<el-form :model="bulkImportRulesForm" :rules="bulkImportRules" ref="bulkImportRulesForm"
						 label-width="80px" class="demo-ruleForm import-box">
					...
					<!-- <el-form-item label="上传操作">
                         <el-upload
                                 ...
                                 :on-exceed="handleExceed"
                                 :file-list="fileList">
                             <el-button size="small" type="primary"><i class="import-icon el-icon-upload"></i>上传
                             </el-button>
                         </el-upload>
                     </el-form-item>-->
					...
					<div class="info-item clear">
						...
						<el-form-item label="联系地址" class="address" prop="address">
							...
							<el-input v-model="updateForm.address" class="width-other">
								<template slot="append"><span @click="openUpMap('address')"><i
				...
				<el-form :model="checkInfoForm" :rules="checkInfoRules" ref="checkInfoForm" label-width="110px"
					...
					<div class="info-item clear">
						...
						<el-form-item label="联系电话" prop="phone">
							<el-input v-model="checkInfoForm.phone"></el-input>
						</el-form-item>
						<el-form-item label="备用电话" prop="">
							<el-input v-model="checkInfoForm.backupPhone"></el-input>
						</el-form-item>
						...
						<el-form-item label="联系地址" class="address required_flag" prop="address">
							...
							<el-input v-model="checkInfoForm.address" class="width-other" prop="address">
								<template slot="append"><span @click="openMap('address')"><i
										class="el-icon-location-outline"></i></span></template>
							</el-input>
						</el-form-item>
					</div>
			...
			<div class="capacity-pop-in info-case" style="padding: 0;">
				...
				<el-form :model="upCheckInfoForm" :rules="upCheckInfoRules" ref="upCheckInfoForm" label-width="110px"
					...
					<div class="info-item clear">
						...
						<el-form-item label="联系电话" prop="phone">
							<el-input v-model="upCheckInfoForm.phone"></el-input>
						</el-form-item>
						<el-form-item label="备用电话" prop="">
							<el-input v-model="upCheckInfoForm.backupPhone"></el-input>
						</el-form-item>
						<el-form-item label="司机类型" prop="driverType">
						...
						<el-form-item label="联系地址"  class="address required_flag" prop="address">
							...
							<el-select v-model="upChAreaId" placeholder="请选择区" filterable>
								<el-option
										v-for="item in areaOption"
										:key="item.regionId"
										:label="item.name"
	...
	export default {
		...
		data() {
			...
			return {
				...
				searchBindTypeOp:[{id:'',name:"全部"},{id:0,name:"未关联"},{id:1,name:"已关联"}],
				searchDriverType:'',
				searchBindType:'',
				searchAuthState:'',
				searchUserName:'',
		...
		methods: {
			initData(){
				...
				let searchData={
					...
					AuthState:this.searchAuthState,
			...
			requestExcelUpload(obj) {
				let company = this.common.fetchData("company");
				obj.companyId = company['pkId'],//this.$route.params['register']['companyId'];// this.$route.params['register']['companyId'];
						this.common.uploadExcelFun(obj, this);
			},
			...
			getPointAddress(e) {
				console.log(e)
				let point = e.point;
				console.info(e.point.lat + "----" + e.point.lng)
            ...
            confirmBtn() {
                ...
                this.$refs.checkInfoForm.validate((valid) => {
                    if (valid) {
                        ...
                        that.$post("companyDriver/addOrUpdateDriver", data).then(function (data) {
                            if (data.code == 0) {
                                that.$message.error(data.msg);
                            } else {
                                // that.$router.go(0)
                                /*that.$refs.checkInfoForm.resetFields();
                                that.$refs.driverForm.resetFields();*/
                                that.clearData(that);
					...
					if (valid) {
						...
						this.$post("companyDriver/addOrUpdateDriver", data).then(function (data) {
							if (data.code == 0) {
								...
								// that.$router.go(0)
								this.$refs.upCheckInfoForm.resetFields();
								that.clearData(that);
								this.upCheckInfo=false;
			...
			submitPicForm() {
				this.$refs.uploadPapersForm.validate((valid) => {
					if (valid) {
						...
						companyDriver.userName = this.driverName;
							...
							if (data.code == 0) {
								...
								that.$post("companyDriver/getInfoById", {driverId: that.driverPkId}).then(function (data) {
									if (data.code == 0) {
										...
										console.log(data.resultData.companDriverVO.companyDriver);
			...
			changeCarFun(pkId, driverType, isEdit) {//弹出更换/关联车辆并初始化下拉框
				this.driverPkId = pkId;
				let that = this;
			...
			submitUpload(){
				...
				if(this.$refs.upload.uploadFiles.length) {
					...
					that.$http.post(that.upLoadExcel, data, config).then(function (response) {
